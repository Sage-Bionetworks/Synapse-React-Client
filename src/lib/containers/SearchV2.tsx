...
import { LockedFilter, QueryContextType } from './QueryContext'
...
class Search extends React.Component<InternalSearchProps, SearchState> {
  ...
  render() {
    ...
    // For study details page: if lockedFilter is defined, remove it from the radio dropdown
    if (searchColumns.length && lockedFilter?.columnName) {
      searchColumns = searchColumns.filter(
        el => el !== lockedFilter?.columnName,
      )
    }
