...
import { FileHandleAssociation, FileHandleAssociateType } from './jsonResponses/FileHandleAssociation'
...
export const testDownloadSpeed = (
  ...
  return new Promise((resolve, reject) => {
     ...
     */
    getEntity(sessionToken, TEST_FILE_ENTITY_ID).then((entity: Entity) => {
        ...
        .then((data: BatchFileResult) => {
          ...
          return getFileHandleContent(fileHandle, presignedUrl).then(
            () => {
              ...
              // bytes/second
              const result = (fileHandle.contentSize / ((new Date().getTime() - startMs - ESTIMATED_CORS_TIME_MS)/1000))
